...
### Added
- **using-superpowers: 多平台工具映射** — 新增 claude-code-tools.md、antigravity-tools.md、pi-tools.md；更新 codex/copilot/gemini 工具映射为上游最新版
- **brainstorming: visual companion 改进** — just-in-time 模式、session key 安全机制、idle timeout、`--open` 参数、Windows 分节支持
- **brainstorming: spec document reviewer** — 同步上游 spec-document-reviewer-prompt.md 格式
- **subagent-driven-development: 合并审查** — 用统一 task-reviewer-prompt.md 替代 spec-reviewer + code-quality-reviewer 双审查
- **subagent-driven-development: 辅助脚本** — 新增 scripts/ 目录（review-package、sdd-workspace、task-brief）
- **requesting-code-review: read-only review 约束** — code-reviewer.md 增加只读审查条款
