...
1.2 确认输出文件路径（默认：项目根目录下 docs/dev/ 目录，文件名格式：{分析目标kebab-case}-deep-analysis.md）
1.3 定位核心文件（Glob 搜索文件名、Grep 搜索关键词/类名/函数名）
1.4 定义核心问题（1-2 个贯穿全文的问题，如"是否规范？""如何权衡？"）
```

**产出：** 核心文件清单 + 核心问题定义 + 输出文件路径
...
4.7 将完整文档写入 Step 1 确认的输出文件路径（使用 Write 工具）
4.8 在 CLI 中仅输出简要摘要：文件路径 + 核心问题 + 关键结论（不超过 10 行）
```

**产出：** 完整文档写入文件 + CLI 简要摘要

**重要：** 最终文档必须使用 Write 工具写入文件，**禁止**在 CLI 中输出完整文档内容。CLI 中只输出简要摘要，引导用户查看文件。
...
**最终文档必须写入文件，禁止在 CLI 中输出完整内容。**

### 文件输出规则

1. **默认路径：** 项目根目录下 `docs/dev/` 目录，文件名格式 `{分析目标kebab-case}-deep-analysis.md`
   - 例：分析 "subprocess usage" → `docs/dev/subprocess-usage-deep-analysis.md`
   - 例：分析 "MCP runtime" → `docs/dev/mcp-runtime-deep-analysis.md`
2. **用户指定路径优先：** 如果用户在参数中指定了输出路径，以用户指定为准
3. **使用 Write 工具写入文件**，不要将完整文档内容输出到 CLI
...
```
📄 深度分析文档已写入：docs/dev/subprocess-usage-deep-analysis.md

核心问题：OpenHarness 的子进程管理是否安全可控？
关键结论：采用三层安全检查 + 沙箱隔离，但残留信号处理竞态风险。
```
...
- [ ] 第七章/第八章列出所有分析到的源码文件
- [ ] 文档已写入文件（使用 Write 工具），CLI 中仅有简要摘要（不超过 10 行）
